/*
 FreeRTOS V6.0.1 - Copyright (C) 2009 Real Time Engineers Ltd.

 ***************************************************************************
 *                                                                         *
 * If you are:                                                             *
 *                                                                         *
 *    + New to FreeRTOS,                                                   *
 *    + Wanting to learn FreeRTOS or multitasking in general quickly       *
 *    + Looking for basic training,                                        *
 *    + Wanting to improve your FreeRTOS skills and productivity           *
 *                                                                         *
 * then take a look at the FreeRTOS eBook                                  *
 *                                                                         *
 *        "Using the FreeRTOS Real Time Kernel - a Practical Guide"        *
 *                  http://www.FreeRTOS.org/Documentation                  *
 *                                                                         *
 * A pdf reference manual is also available.  Both are usually delivered   *
 * to your inbox within 20 minutes to two hours when purchased between 8am *
 * and 8pm GMT (although please allow up to 24 hours in case of            *
 * exceptional circumstances).  Thank you for your support!                *
 *                                                                         *
 ***************************************************************************

 This file is part of the FreeRTOS distribution.

 FreeRTOS is free software; you can redistribute it and/or modify it under
 the terms of the GNU General Public License (version 2) as published by the
 Free Software Foundation AND MODIFIED BY the FreeRTOS exception.
 ***NOTE*** The exception to the GPL is included to allow you to distribute
 a combined work that includes FreeRTOS without being obliged to provide the
 source code for proprietary components outside of the FreeRTOS kernel.
 FreeRTOS is distributed in the hope that it will be useful, but WITHOUT
 ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
 FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for
 more details. You should have received a copy of the GNU General Public 
 License and the FreeRTOS license exception along with FreeRTOS; if not it 
 can be viewed here: http://www.freertos.org/a00114.html and also obtained 
 by writing to Richard Barry, contact details for whom are available on the
 FreeRTOS WEB site.

 1 tab == 4 spaces!

 http://www.FreeRTOS.org - Documentation, latest information, license and
 contact details.

 http://www.SafeRTOS.com - A version that is certified for use in safety
 critical systems.

 http://www.OpenRTOS.com - Commercial support, development, porting,
 licensing and training services.
 */

/*******************************************************************
 * This example has been heavily modified from the original to model the 
 * examples in the RTOS manual 
 * NJG 02/12/09 */

/* Standard includes. */
#include <stddef.h>
#include <stdio.h>
#include <string.h>
#include <unistd.h>
#include <stdlib.h>
#include <math.h>

/* Scheduler includes. */
#include "FreeRTOS.h"
#include "task.h"
#include "queue.h"
#include "semphr.h"

#include "system.h"
#include "altera_avalon_pio_regs.h"
#include "altera_avalon_lcd_16207_regs.h"
#include "alt_types.h"
#include "altera_avalon_timer_regs.h"
#include "pwm.h"
#include "emu.h"
#include "lcd.h"
#include "ecdRTOS.h"
/* Altera HAL includes for timestamp timer */
#include "sys/alt_timestamp.h"

/*state machine states*/
#define NO_BUTTON        0
#define MOVE_RIGHT       1
#define MOVE_LEFT        2
#define MOVE_UP          3
#define MOVE_DOWN        4
#define MOVE_FORWARD     5
#define MOVE_BACKWARD    6
#define MOVE_GRIPPER     7
#define RECORD           8
#define PLAY_PAUSE       9
#define PLAYBACK        11
#define RESET            15

#define MOVE_STEP     3

#define RESET_KEY 1

//PWM Globals  //BSEG = base shoulder elbow gripper
int PWMBase[4] =
  { MY_PWM_0_BASE, MY_PWM_1_BASE, MY_PWM_2_BASE, MY_PWM_3_BASE }; 
int PWMMax[4] = {BASE_MAX,SHOULDER_MAX,ELBOW_MAX,GRIPPER_MAX};
int PWMMin[4] = {BASE_MIN,SHOULDER_MIN,ELBOW_MIN, GRIPPER_MIN};
  
//LCD File descriptor
FILE* pLCDFp;

//Holds the data for the device
DeviceData* deviceData;

int storeOffset =0,playbackOffset = 0, playback =0, record =0; 
int storedBSEG[300];

int main(void)
{
  //initialistation
  deviceData = malloc (sizeof(DeviceData));
  memset(&storedBSEG,0,sizeof(storedBSEG));   
  InitialiseLCDScreen();
  InitialisePWM();
  usleep(200); 
  
  //tasks are created with parameters from ecdRTOS.h  
  xTaskCreate(PWMWriteTask, "pwm write task", TASK_STACK_DEPTH, (void *) PWM_TASK_ID, PWM_TASK_PRIORITY, NULL);
  xTaskCreate(KeyPadReadTask, "Keypad Read task", TASK_STACK_DEPTH, (void *) KEYPAD_TASK_ID, KEYPAD_TASK_PRIORITY, NULL);
  
  
  /* Start the scheduler. */
  vTaskStartScheduler();

  /* Will only reach here if there is insufficient heap available to start
   the scheduler. */
  for (;;)
    ;
}
/*
clear screen function, this will move the cursor to the begining of each
line and then clear all characters on the line
*/
void ClearLCDScreen()
{
  fprintf(pLCDFp, CLEAR_SCREEN);
  usleep(100);
}

/*
Writes characters to the specified line(destLine 1=top, 2=bottom) of the LCD screen
@pDestLine: The line to write to on the LCD Screen
@apBuffer: string to write to the LCD
*/
void WriteToLCDLine(int aDestLine, char* apBuffer)
{
  fprintf(pLCDFp, "\e[%d;1H\e[K%s", aDestLine, apBuffer);
  usleep(1000);
}

/*
 Task to recieve keypad presses and to create packets for other tasks to deal with
 @apParameters: used for passing in parameters, not implemented   
*/
void KeyPadReadTask(void* apParameters)
{
    
  char* lCDTopLineBuffer;
  char* lCDBotLineBuffer;
  int openClose = 1;
  int state, iA, prevState;  
  int newBSEG[4];
  
  for (;;)
    {
      IOWR_ALTERA_AVALON_PIO_DATA(LEDS_BASE, 1);
      //read from keypad  
      state = IORD_ALTERA_AVALON_PIO_DATA(COLLS_BASE);
      state &= 0x0F; //mask out stale bit
            
      //read switches, for debugging w/o keypad
      //state = IORD_ALTERA_AVALON_PIO_DATA(SWITCHES_BASE);  
      
      //debugging print out list current PWM values
    // printf("key curr: B=%d S=%d E=%d G=%d\n", deviceData->mpCurrentBSEG[BASE], deviceData->mpCurrentBSEG[SHOULDER], 
    //                              deviceData->mpCurrentBSEG[ELBOW], deviceData->mpCurrentBSEG[GRIPPER]);
      //copy across current PWM values into new array to modify
      for (iA = 0; iA < 4; iA++)
        {
          newBSEG[iA] = deviceData->mpCurrentBSEG[iA];
        }     
       //after the key is read, send reset signal to keypad handler
        IOWR_ALTERA_AVALON_PIO_DATA(ROWS_BASE, RESET_KEY); 
        //printf("keypad state = %x\n", state);
      switch (state)
        {           
     
      // Emergency Stop
     // case EMERGENCY:
     //  {         
          // Stop ALL movements immediatly
          // Do not allow movement until some condition has been met
          //break;
      //  }
     
        // Reset
     case RESET:
        {
          // Move the arm to the central position, clear flags adn stored values
          newBSEG[BASE] = DEFAULT_BASE;
          newBSEG[SHOULDER] = DEFAULT_SHOULDER;
          newBSEG[ELBOW] = DEFAULT_ELBOW;          
          lCDTopLineBuffer = "Reset";
          lCDBotLineBuffer = ". . . . .";
          playback = 0;
          record = 0;
          storeOffset = 0;
          playbackOffset = 0;
          memset(&storedBSEG,0,sizeof(storedBSEG));          
          break;
       }
       
       
        // Move Left (anti-clockwise)
      case MOVE_LEFT:
        {          
          lCDTopLineBuffer = "Arm Moving Left";
          newBSEG[BASE] += MOVE_STEP;
          lCDBotLineBuffer = CreateBSEGString(newBSEG);
          // Cause the arm to move to the left
          break;
        }
        
        
        // Move Right (clockwise)
      case MOVE_RIGHT:
        {
          lCDTopLineBuffer = "Arm Moving Right";
          newBSEG[BASE] -= MOVE_STEP;
          lCDBotLineBuffer = CreateBSEGString(newBSEG); 
          // Cause the arm to move to the right
          break;
        }           
        // Move Up
      case MOVE_UP:
        {
          lCDTopLineBuffer = "Arm Moving Right";
          newBSEG[SHOULDER] += MOVE_STEP;
          lCDBotLineBuffer = CreateBSEGString(newBSEG);
          // Cause the arm to move upwards
          break;
        }        
        //Move Down        
      case MOVE_DOWN:
        {
          lCDTopLineBuffer = "Arm Moving Right";
          newBSEG[SHOULDER] -= MOVE_STEP;
          lCDBotLineBuffer = CreateBSEGString(newBSEG);
          // Cause the arm to move downwards
          break;
        }
        //Move Forwards
      case MOVE_FORWARD:
        {
          lCDTopLineBuffer = "Arm Moving fwd";
          newBSEG[ELBOW] += MOVE_STEP;
          lCDBotLineBuffer = CreateBSEGString(newBSEG);
          // Cause the arm to move forwards
          break;
        }                
        // Move Backwards        
      case MOVE_BACKWARD:
        {
          lCDTopLineBuffer = "Arm Moving bck";
          newBSEG[ELBOW] -= MOVE_STEP;
          lCDBotLineBuffer = CreateBSEGString(newBSEG);
          // Cause the arm to move backwards
          break;
        }  
        // Open/close the gripper
      case MOVE_GRIPPER:
        {
          
          lCDBotLineBuffer = ".....";          
          if (newBSEG[GRIPPER] == PWMMax[GRIPPER]-1)
          {
            lCDTopLineBuffer = "Gripper Closed";
            newBSEG[GRIPPER] = PWMMin[GRIPPER]+1;
          }
          else
          {
            lCDTopLineBuffer = "Gripper Open";
            newBSEG[GRIPPER] = PWMMax[GRIPPER]-1;
          }
          break;
        }        
        // Record Path        
      case RECORD:
        {
          lCDTopLineBuffer = "Recording Path";
          lCDBotLineBuffer = ".....";
          record = 1;          
          break;
        }
       //initiates playback, --- pause during playback not implemented 
       //Turns off record flag and sets playback flag
      case PLAY_PAUSE:
        {
          lCDTopLineBuffer = "Replaying Path";
          lCDBotLineBuffer = ".....";
          playback =1;
          record = 0;     
          break;
        }
        //possible extentions
        // Menu Function
        /*
      case MENU:
        {
          //TODO: think about menu options
          lCDTopLineBuffer = "Menu L/R Scrolls";
          lCDBotLineBuffer = "1: Option One";

          // Display a menu which allows control over settings etc.

          break;
        }
        */
        /*
      case EXIT:
        {
          lCDTopLineBuffer = "Exiting";
          lCDBotLineBuffer = ". . . . ";
          break;
        }
        */
        //do not change display buffers if no button is pressed 
      default:
        {
          //printf("No valid option was selected\n");
          lCDTopLineBuffer = deviceData->mpCurrentLCDTopLineBuffer;
          lCDBotLineBuffer = deviceData->mpCurrentLCDBotLineBuffer;
        }
        }//endcase
      //do not change display buffers if no button is pressed  
      if (state != 0)
      {      
        WriteToLCDLine(1, lCDTopLineBuffer);    
        WriteToLCDLine(2, lCDBotLineBuffer);
      }
      deviceData->mpCurrentLCDTopLineBuffer = lCDTopLineBuffer;
      deviceData->mpCurrentLCDBotLineBuffer = lCDBotLineBuffer;
      
      //write modified array to device array
      for (iA = 0; iA < 4; iA++)
      {
        deviceData->mpNewBSEG[iA] = newBSEG[iA];  
      }
      //light record LED if record flag is active
      if (record == 1)
      {
        IOWR_ALTERA_AVALON_PIO_DATA(LEDS_BASE, 128);
      }
      //light playback LED if playback active
      if (playback == 1)
      {
        IOWR_ALTERA_AVALON_PIO_DATA(LEDS_BASE, 32);
      }
      
      vTaskDelay(KEYPAD_TASK_DELAY);
     // }//end If oldbutton!=state      
    }//end for    
}//endtask

/*
 This function creates the string that will be written to the bottom fo the second line
 @apNewBSEG - pointer to the int array that holds the values that will be written to the PWMs 
 */
char* CreateBSEGString(int* apNewBSEG)
{
  char* pPositionBuffer;
  pPositionBuffer = (char *) malloc(sizeof(char[MAX_BUFFER_LENGTH]));
  
  sprintf(pPositionBuffer, "B=%d S=%d E=%d\n", apNewBSEG[BASE],
      apNewBSEG[SHOULDER], apNewBSEG[ELBOW]);
  return pPositionBuffer;
}

/*
 Initialises the LCD Screen and buffers, only called once.  Clears the screen 
 */
void InitialiseLCDScreen()
{
  pLCDFp = fopen("/dev/lcd", "w+");
  deviceData->mpNewLCDTopLineBuffer = "Starting System";
  deviceData->mpNewLCDBotLineBuffer = ".......";
  deviceData->mpCurrentLCDTopLineBuffer = "Starting System";
  deviceData->mpCurrentLCDBotLineBuffer = "......";
}
/*
Initialises the PWMs, using the BSEG order in the pwmbase array,
sets both 'new' and 'current' member arays to default values
*/
void InitialisePWM()
{
  int i; 
  for (i = 0; i < JOINTS; i++)
    {
      IOWR_ALTERA_AVALON_PWM_CLK_DIV(PWMBase[i], 100);
      IOWR_ALTERA_AVALON_PWM_PERIOD(PWMBase[i], 1000);
      IOWR_ALTERA_AVALON_PWM_CONTROL(PWMBase[i], 1);
    }
    
  deviceData->mpNewBSEG[BASE] = DEFAULT_BASE;
  deviceData->mpNewBSEG[ELBOW] = DEFAULT_ELBOW;
  deviceData->mpNewBSEG[SHOULDER] = DEFAULT_SHOULDER;
  deviceData->mpNewBSEG[GRIPPER] = DEFAULT_GRIPPER;
  
  deviceData->mpCurrentBSEG[BASE] = DEFAULT_BASE;
  deviceData->mpCurrentBSEG[ELBOW] = DEFAULT_ELBOW;
  deviceData->mpCurrentBSEG[SHOULDER] = DEFAULT_SHOULDER;
  deviceData->mpCurrentBSEG[GRIPPER] = DEFAULT_GRIPPER;
  
  
 // printf("B=%d S=%d E=%d G=%d\n", deviceData->mpNewBSEG[BASE], deviceData->mpNewBSEG[SHOULDER], 
  //                                deviceData->mpNewBSEG[ELBOW], deviceData->mpNewBSEG[GRIPPER]);
}
/*
  The task responsible for writing to the PWMs
  The order and organisation follows BSEG - Base, shoudler, elbow and gripper
  Indexes through arrays and writes to the PWM with the value in the 
  corresponding value array for example - PWMbase[1] with be written by BSEG[1] 
  
  Playback functionality included within this task, modifies the location that 
  values are retrieved that write to the PWMs.
  Values are stored in a one dimensional array of 400 elements.  Two offsets are used
  storeOffset, playbackOffset.  Each are sued to index the stredBSEG array and in  in con
  conjunction with the index retrieves/stores values sequentially.  index is multiplied by
  four when retrieving values to skip over already read values  
  @pParamters - paramters passed via scheduler, not used.
*/
void PWMWriteTask(void* pParameters)
{
   int iA = 0;
   int PWMBSEG[JOINTS] = {0,0,0,0};      
   for(;;){
    //light PWM LED
   IOWR_ALTERA_AVALON_PIO_DATA(LEDS_BASE, PWM_TASK_ID);
  
   //determine mode, if playback active look at stored values 
   if (playback == 1)
     {          
        for (iA = 0; iA < JOINTS; iA++)       
        {        
          PWMBSEG[iA] = storedBSEG[(iA+(playbackOffset*JOINTS))];         
          playbackOffset += 1;
        }       
      }
    else
      {
        for (iA = 0; iA < JOINTS; iA++)
        {      
          PWMBSEG[iA] = deviceData->mpNewBSEG[iA];
        } 
      }  
       
       
    for (iA = 0; iA < JOINTS; iA++)
     {         
        if (PWMBSEG[iA] > PWMMin[iA] && PWMBSEG[iA] < PWMMax[iA]) // if within raqnge
        {
           
           if (playback == 0)
           { 
             IOWR_ALTERA_AVALON_PWM_DUTY(PWMBase[iA], PWMBSEG[iA]); //write to PWM
             deviceData->mpCurrentBSEG[iA] = PWMBSEG[iA];
            /*
             if (iA == GRIPPER)
             {
                printf("pwm: G=%d\n", deviceData->mpNewBSEG[GRIPPER]);
             }
             */
           }
           if (record == 1)
           {
             //increment offset and save current pwm values                       
             storedBSEG[(iA+storeOffset*JOINTS)] = PWMBSEG[iA];
             
             if (storeOffset >= 100)
             {
                record = 0;                
             }
             if (iA == GRIPPER)
             { 
                printf("Store offset = %d B=%d S=%d E=%d G=%d\n", storeOffset, 
                        storedBSEG[BASE+(storeOffset*JOINTS)], 
                        storedBSEG[SHOULDER+(storeOffset*JOINTS)], 
                        storedBSEG[ELBOW+(storeOffset*JOINTS)], 
                        storedBSEG[GRIPPER+(storeOffset*JOINTS)]);
                record = 0;
                storeOffset += 1;
                
             }          
           }
           else if (playback == 1) //playback mdoe activated
           {
            IOWR_ALTERA_AVALON_PWM_DUTY(PWMBase[iA], PWMBSEG[iA]); //write to PWM
            usleep(5000);
           }           
        }
        else
        {
           //retain old value if outside of range
           IOWR_ALTERA_AVALON_PWM_DUTY(PWMBase[iA], deviceData->mpCurrentBSEG[iA]);
        } 
        if (playbackOffset >= storeOffset)
        {
          //end playback when offsets meet or pass
          playback = 0;
          playbackOffset = 0;
        }     
      }      
     vTaskDelay(PWM_TASK_DELAY);
   }    
    
}
